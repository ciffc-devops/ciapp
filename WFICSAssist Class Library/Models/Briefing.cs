...
namespace WFICSAssist_Class_Library.Models
{
    ...
    public class Briefing : ICloneable
    {
        ...
        private void setCmdAndCommsPlanItems(WFIncident task)
        {
            ...
            if (task.allUrgencyCalculations.Where(o => o.OpPeriod == OpPeriod).Any())
            {
                StringBuilder urgency = new StringBuilder(); //Urgency Rating
                UrgencyCalculation calc = task.allUrgencyCalculations.Where(o => o.OpPeriod == OpPeriod).First();
                //AllSections.Where(o => o.sectionID == commandcommunicationsID).First().allItems.Where(o => o.itemName == "Urgency Rating").First().itemValue = calc.FullUrgencyText;
                AllSections.Where(o => o.sectionID == situationID).First().allItems.Where(o => o.itemName == "Urgency Rating").First().itemValue = calc.FullUrgencyText;
